//! Copied directly from https://github.com/phip1611/libbruteforce
...
/// the alphabet).
pub fn indices_create(max_length: u8, min_length: u8) -> Box<[isize]> {
...
// #[cfg(test)]
// mod tests {
//     use crate::charsets::combo_count;
//     use crate::parameters::CrackParam;
...
//         let charset: Box<[char]> = Box::from(['a', 'b', 'c']);
//         let min_len: u8 = 0;
//         let max_len: u8 = 3;
//         let script = "java".into();
//         let args = Box::from(["Java".into()]);
//         let look_for_output = "Correct!".into();
//         let spawn = Spawn::new(script, args, look_for_output);
//         let cp = CrackParam::new(charset.clone(), min_len, max_len, spawn);
//         let mut indices = indices_create(max_len, min_len);
//         // should make -1 -1 -1 to 2 2 2
//         // minus one because we are already at the first element (-1, -1, -1)
//         let steps = combo_count(cp.unwrap()) - 1;
//         indices_increment_by(&charset, &mut indices, steps).unwrap();
//         for i in 0..max_len {
//             assert_eq!(indices[i as usize], (charset.len() - 1) as isize)
//         }
//     }
// }
